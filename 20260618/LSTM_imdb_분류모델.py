...
from tensorflow.keras.layers import Embedding, LSTM, Dense, Input, SimpleRNN  # type: ignore
...
# model.add(SimpleRNN(hidden_units))  # 200
model.add(LSTM(hidden_units))  # 100(입력) * 128(뉴런) + 128(바이어스)
...
# 최상의 검증 점수를 낸 모델 저장
checkpoint_cb = ModelCheckpoint("/home/sm/tf_env/20260617/best-lstm-model.keras", save_best_only=True, verbose=1)
